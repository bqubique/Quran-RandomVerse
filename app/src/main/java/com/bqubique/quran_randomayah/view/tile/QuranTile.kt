...
@AndroidEntryPoint
class QuranTile : TileService() {
    private val serviceJob = Job()
    private val serviceScope = CoroutineScope(Dispatchers.IO + serviceJob)

    @Inject
    lateinit var quranApi: VerseApi
    ...
    private fun layout(ayah: EnglishVerseModel, deviceParameters: DeviceParametersBuilders.DeviceParameters) =
        ...
        do {
            runBlocking {
                CoroutineScope(Dispatchers.IO).launch {
                    englishVerseResponse = quranApi.getRandomEnglishVerse().body()!!
                }.join()
            }
